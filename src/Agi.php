 ...
 * AGI client class.
 * 
 * This class is a modified version of the PHP AGI client
 * (based on phpagi.php, v 2.20)
 *
 * Copyright (c) 2016 <contact@agiphlow.com>
 * All Rights Reserved.
 *
...
class Agi
{
  ...
  const STATE_OFFHOOK =  2;
  const STATE_DIALING =  3;
  const STATE_RING =  4;
  const STATE_RINGING =  5;
  const STATE_UP =  6;
  const STATE_BUSY =  7;
  const STATE_DIALING_OFFHOOK =  8;
  const STATE_PRERING =  9;
  ...
  public function __construct($config=array())
  {
    ...
    if ($this->config['use_default_error_handler']) {
      set_error_handler(array($this, 'errorHandler'));
    }

    // open stdin & stdout
    $this->in = isset($this->config['io_proxy']['in'])
      ? $this->config['io_proxy']['in']
      : fopen('php://stdin', 'r');
    $this->out = isset($this->config['io_proxy']['out'])
      ? $this->config['io_proxy']['out']
      : fopen('php://stdout', 'w');
  ...
  protected function openAudio()
  {
    // open audio if eagi detected
    if(!isset($this->request['agi_enhanced']) 
      || $this->request['agi_enhaced'] != '1.0'
    ) {
      return;
    }
  ...
  public function channel_status($channel='')
  {
    ...
    case self::STATE_DOWN: $ret['data'] = 'Channel is down and available'; break;
    case self::STATE_RESERVED: $ret['data'] = 'Channel is down, but reserved'; break;
    case self::STATE_OFFHOOK: $ret['data'] = 'Channel is off hook'; break;
    case self::STATE_DIALING: $ret['data'] = 'Digits (or equivalent) have been dialed'; break;
    case self::STATE_RING: $ret['data'] = 'Line is ringing'; break;
    case self::STATE_RINGING: $ret['data'] = 'Remote end is ringing'; break;
    case self::STATE_UP: $ret['data'] = 'Line is up'; break;
    case self::STATE_BUSY: $ret['data'] = 'Line is busy'; break;
    case self::STATE_DIALING_OFFHOOK: $ret['data'] = 'Digits (or equivalent) have been dialed while offhook'; break;
    case self::STATE_PRERING: $ret['data'] = 'Channel has detected an incoming call and is waiting for ring'; break;
  ...
  public function fastpass_say_digits(&$buffer, $digits, $escape_digits='')
  {
    ...
    if($buffer == '' || $proceed)
    {
      ...
      if($res['code'] == self::AGI_OK && $res['result'] > 0)
        $buffer .= chr($res['result']);
      return $res;
    }
    return array('code'=>self::AGI_OK, 'result'=>ord($buffer{strlen($buffer)-1}));
  }
  ...
  public function fastpass_stream_file(&$buffer, $filename, $escape_digits='', $offset=0)
  {
    ...
    return array('code'=>self::AGI_OK, 'result'=>ord($buffer{strlen($buffer)-1}), 'endpos'=>0);
  }
    ...
    if(is_null($max_digits) || strlen($buffer) < $max_digits)
    {
      ...
      else
      {
        while(is_null($max_digits) || strlen($buffer) < $max_digits)
        {
          ...
          if($res['code'] != self::AGI_OK) return $res;
          if($res['result'] == ord('#')) break;
          $buffer .= chr($res['result']);
        }
      }
    }
    return array('code'=>self::AGI_OK, 'result'=>$buffer);
  }
  ...
  public function text2wav($text, $escape_digits='', $frequency=8000)
  {
    ...
    $hash = md5($text);
    $fname = $this->config['tempdir'] . DIRECTORY_SEPARATOR;
    ...
    foreach(glob($this->config['tempdir'] . DIRECTORY_SEPARATOR . 'text2wav_*') as $file)
  ...
  public function swift($text, $escape_digits='', $frequency=8000, $voice=NULL)
  {
    ...
    // clean up old files
    $delete = time() - 2592000; // 1 month
    foreach(glob($this->config['tempdir'] . DIRECTORY_SEPARATOR . 'swift_*') as $file)
      if(filemtime($file) < $delete)
        unlink($file);
  ...
  protected function evaluate($command)
  {
    ...
    $ret['result'] = NULL;
    $ret['data'] = '';
    if($ret['code'] != self::AGI_OK) // some sort of error
    {
      $ret['data'] = $str;
      $this->conlog(print_r($ret, true));
    }
    else // normal self::AGI_OK response
    {
  ...
  /**
   * Default error handler for phpagi.
   *
  ...
  public function errorHandler($level, $message = '', $file = '', $line = '', $context = '')
  {
    ...
    $this->conlog(
      "PHP Error [$level]: $message [$file:$line] ($context)"
    );
  }
